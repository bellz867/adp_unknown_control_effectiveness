...
cd('Plots and Conditions\VarFTComparisontoConf\VarFTDesigned__27-Sep-2018')
load Outputs.mat
uDes = u;
xDes = x;
vHatDes = VHat;
...
if or(CompAll==1, CompNoS==1)
    %Get input from result without added terms
    cd('Plots and Conditions\VarFTComparisontoConf\VarFTPrev__27-Sep-2018')
    load Outputs.mat
    uNoS = u;
    xNoS = x;
    vHatNoS = VHat;
...
%     title('(a)','interpreter','latex'); %const FT Plot
    title('(b)','interpreter','latex'); %var Ft Plots
end
xlabel('time (sec)','interpreter','latex');

ylim([MinY-0.25,MaxY+1])
...
if CompAll==1
    ...
    l  = legend([p1Mark(1),p1Mark(2),p1Mark(3)],'Developed controller', 'Complementary controller', 'Controller with $S(x)=0$');
...
elseif and(CompAll==0, CompNoS==1)
    ...
    l  = legend([p1Mark(1),p1Mark(2)],'Developed controller', 'Controller with $S(x)=0$');
    
    MinY = min(min(min([xDes(:,1:TF);xNoS(:,1:TF)])));
    MaxY = max(max(max([xDes(:,1:TF);xNoS(:,1:TF)])));
...
set(gca,'Position',[0.1300, 0.4975 0.7750, 0.3000]);
set(gca,'XTickLabel',[]);
set(gca,'fontsize',20);
set(gca,'FontName','Times New Roman');
set(gca,'XGrid','on');
...
set(gca,'Position',[0.1300, 0.1800, 0.7750, 0.3000]);
...
%     H = title('(c)','interpreter','latex'); %Const Fault Plots
    ...
    set(H, 'Position',[7.5000 13.5 0],'fontsize',20);
end
xlabel('time (sec)','interpreter','latex');
...
% print -djpeg StateComparison.jpg
...
elseif and(CompAll==0, CompNoS==1)
    ...
    l  = legend([pMark(1),pMark(2)],'Developed Controller','Controller Without Added Terms');
...
% print -djpeg costComparison.jpg