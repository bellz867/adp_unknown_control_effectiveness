...
if (sat == 0)
    addInput = auxdata.InputKp1*(y'*y/(1+auxdata.InputKp2*(y'*y)))*y;
...
%         sat^2*auxdata.R(2,2)*log(1 - u(2)^2/sat^2);
    uCost = 2*sat*u'*auxdata.R*atanh(u/sat)+ sat^2*auxdata.Rvec*log(ones(m,1)-u.^2./sat^2);

%     Ga =
%     -sigPrime*(sat*g*sign((1/(sat))*(0.5*(auxdata.R\g')*(sigPrime'*WaHat+auxdata.InputKp*x)))+g*u); %Usig signum
    Ga = -sigPrime*(sat*g*tanh(1/(0.000001)*g'*(sigPrime'*WaHat+addInput))+g*u);  %Using close approximation of signum
%     Ga = zeros(size(Ga));
